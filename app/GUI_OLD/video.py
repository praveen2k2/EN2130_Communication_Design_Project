    ...
    def show(self):
        ...
        def browse_video():
            file_path = filedialog.askopenfilename(
                filetypes=[("All Files", "*.*"), ("Video Files", "*.mp4;*.avi;*.mkv;*.ts")]
            )
            if file_path:
        ...
        # Send button
        def send_video():
            heading_new = tk.Label(self.root, text="Sending Video...", font=("Arial", 15))
            heading_new.pack(pady=20)

            progress_bar = ttk.Progressbar(self.root, orient="horizontal", length=300, mode="determinate")
            progress_bar.place(x=100, y=200)
            ...
            def update_progress():
                ...
                messagebox.showinfo("Success", "Video successfully sent!")
