...
class ImageFrame:
    ...
    def show(self):
        ...
        def display_thumbnail(file_path):
            ...
            self.thumbnail_canvas = tk.Canvas(self.root, width=150, height=150, bg="white", highlightthickness=1,
        ...
        def save_file():
            self.tmp_file_path = filedialog.asksaveasfilename(
            ...
            if self.tmp_file_path:
                # Save the file as the selected type
                try:
                    with open(self.tmp_file_path, "rb") as tmp_file:
                        data = tmp_file.read()

                    with open(self.tmp_file_path, "wb") as new_file:
                        new_file.write(data)
        ...
        send_button = ttk.Button(button_frame, text="Send", command=send_image, state=tk.DISABLED, width=20,
                                 style="Custom.TButton")
        send_button.grid(row=0, column=1, padx=10)

        # Cancel button
        cancel_button = ttk.Button(button_frame, text="Cancel", command=self.switch_back_callback, width=20,
                                   style="Custom.TButton")
