...
import Telelink_host_epy_block_0 as epy_block_0  # embedded python block
import sip
...
def snipfcn_snippet_0(self):
    ...
    if __name__ == "__main__":
        ...
        cmd = r'vlc v4l2:///dev/video0 --sout="#transcode{vcodec=mp2v,vb=400,scale=0.25,acodec=none,scodec=none}:duplicate{dst=file{dst=./im.ts,no-overwrite},dst=display}" --sout-all --sout-keep'
...
class Telelink_host(gr.top_block, Qt.QWidget):

    def __init__(self, MTU=1500):
        gr.top_block.__init__(self, "QPSK_Streaming", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("QPSK_Streaming")
        ...
        self.settings = Qt.QSettings("GNU Radio", "Telelink_host")
...
def main(top_block_cls=Telelink_host, options=None):
    if options is None:
